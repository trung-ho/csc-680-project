...
try {
  ...
  } else if ((isset($_POST['id'])) && (is_numeric($_POST['id']))) { // Form submission.      #1
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($sure == 'Yes') { // Delete the record.
      ...
      if (mysqli_stmt_affected_rows($q) == 1) { // It ran OK
        // Print a message:
        echo '<h4 class="text-center alert alert-primary">The record has been deleted.</h4>';
      ...
      echo '<h4 class="text-center alert alert-primary">The user has NOT been deleted as you requested</h4>';
    }
    ...
    <form action="delete_user.php" method="post" name="deleteform" id="deleteform">
    	<div class="form-group row">
	    	...
	    	<div class="col-sm-8" style="padding-left: 70px;">
			    ...
			    <input id="submit-yes" class="btn alert-danger" type="submit" name="sure" value="Yes"> - 
			    <input id="submit-no" class="btn alert-info" type="submit" name="sure" value="No">
    		</div>
    	</div>
    </form>
<?php
    } else { // Not a valid user ID.
      echo '<p class="text-center alert alert-danger">This page has been accessed in error.</p>';
      echo '<p>&nbsp;</p>';
    }
