...
try {
  ...
  if (empty($password1trim)) { //                                             #7
    ...
    if (!preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[#$@!%&*?])[A-Za-z\d#$@!%&*?]{8,12}$/', $password1trim)) { //                        #8
  ...
  if (empty($errors)) { // If everything's OK.
    ...
    if (mysqli_num_rows($result) == 0) { //The email address has not been registered
      ...
      mysqli_stmt_bind_param($q, 'sssssssssssss', $titletrim, $first_nametrim, $last_nametrim, $emailtrim, $hashed_password, $classtrim, $address1trim, $address2trim, $citytrim, $state_countrytrim, $zcode_pcodetrim, $phonetrim, $secrettrim);
      // execute query
      mysqli_stmt_execute($q);
      if (mysqli_stmt_affected_rows($q) == 1) {
        // header("location: register-thanks.php?class=" . $classtrim);
        echo "<script type='text/javascript'> document.location = 'finished-sign-up.php'; </script>";
      } else {
