...
try {
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) { // If everything's OK.                                         #2    
      ...
      mysqli_stmt_bind_param($q, 'ssssssssss', $titletrim, $first_nametrim, $last_nametrim, $address1trim, $address2trim, $citytrim, $state_countrytrim, $zcode_pcodetrim, $phonetrim, $id);
      // execute query
      
      mysqli_stmt_execute($q);
      // var_dump($titletrim, $first_nametrim, $last_nametrim, $address1trim, $address2trim, $citytrim, $state_countrytrim, $zcode_pcodetrim, $phonetrim, $id);
      if (mysqli_stmt_affected_rows($q) == 1) { // Update OK
        
        // Echo a message if the edit was satisfactory:
        echo '<h3 class="alert alert-success">The user has been edited.</h3>';
...
?>
	...
	<form action="edit_address.php" method="post" name="editform" id="editform">
	    ...
	    <div class="col-sm-8">
	      ...
	      title="Alphabetic, period and space max 12 characters"
	       value="<?php if (isset($row['title'])) echo htmlspecialchars($row['title'], ENT_QUOTES); ?>" >
	    </div>
	  </div>
	<div class="form-group row">
	    <label for="first_name" class="col-sm-4 col-form-label text-right">First Name*:</label>
	    <div class="col-sm-8">
	      <input type="text" class="form-control" id="first_name" name="first_name" title="Alphabetic and space only max of 30 characters"
	  ...
	  <div class="form-group row">
		...
		<div class="form-group row">
	    <div class="col-sm-12">
	    	<input id="submit" class="btn primary-btn" type="submit" name="submit" value="Edit User">
			</div>
		</div>
	</form>
<?php
