  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) { // If everything's OK.                                     
      ...
      if (mysqli_num_rows($result) == 0) { // e-mail does not exist in another record
        ...
        mysqli_stmt_bind_param($q, 'sssssi', $first_name, $last_name, $emailtrim, $class, $paid, $id);
        // execute query
...
<form action="edit_user.php" method="post" name="editform" id="editform">
	<div class="form-group row">
      ...
      placeholder="First Name" maxlength="30" required
       value=
        "<?php
          if (isset($row1['first_name'])) echo htmlspecialchars($row1['first_name'], ENT_QUOTES);
        ?>" >
    </div>
	</div>
	<div class="form-group row">
    <label for="last_name" class="col-sm-4 col-form-label"> Last Name:</label>
      ...
      value=
        "<?php
          if (isset($row1['last_name'])) echo htmlspecialchars($row1['last_name'], ENT_QUOTES);
        ?>" >
    </div>
	</div>
	<div class="form-group row">
     <label for="email" class="col-sm-4 col-form-label">E-mail:</label>
      ...
      placeholder="E-mail" maxlength="60" required
       value=
        "<?php
          if (isset($row1['email'])) echo htmlspecialchars($row1['email'], ENT_QUOTES); ?>" >
    </div>
	</div>
  <div class="form-group row">
          ...
          <?php
            ...
            if ($result) { // If it ran OK, display the records.
              ...
              for ($j = 0, $i = 0; $j < 5; $j++, $i = $i + 2) {
                ...
                if ((isset($row1['class'])) && ($row1['class'] == $row[$i])) {
            ?>
               selected
              <?php
                  }
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-8">
      ...
      value="<?php
        echo htmlspecialchars($row1['paid'], ENT_QUOTES);
      ?>">
    </div>
  </div>
